...
def infer_spatial_role(profile: DatasetSemanticProfile) -> SpatialRole:    
    ...
    if any((c.raw_type and c.raw_type.lower() in ("geometry", "polygon")) 
           or (c.spatial_resolution and c.spatial_resolution.lower() in ("region", "area"))        
           for c in spatial_cols) or any_name_matches(BOUNDARY_KEYWORDS):       
            return "boundary"    
